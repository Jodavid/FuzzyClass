...
#' Fuzzy Exponential Naive Bayes Classifier
#'
#' \code{ExpNBFuzzyParam} Fuzzy Exponential Naive Bayes Classifier
#'
#'
...
ExpNBFuzzyParam.default <- function( train, cl, metd = 1, cores = 2){
  ...
  Parameters_lambda <- lapply(1:length(lambdas),function(i){ #Pra grupos
    lapply(1:length(lambdas[[1]]),function(k){ #Para dimensao
      round(t(sapply(1:length(alpha),function(j){
        c(( sqrt(N) * ( lambdas[[i]][k] ) ) / ( sqrt(N) + qnorm(1-alpha[j]/2)  ),
  ...
  # -------------------------------------------------------
  structure(list(Parameters_lambda = Parameters_lambda,
                 lambdas = lambdas,
                 cols = cols,
                 M = M,
                 alpha = alpha,
